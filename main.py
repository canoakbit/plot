...
df['Predicted Yield'] = (df['High'] + df['Low']) / 2 + np.random.uniform(-0.1, 0.1, size=len(df))
...
result_file_path = 'data\\5year_result.csv'
...
plt.plot(df['Date'], df['Predicted Yield'], label='Predicted Yield', marker='x', linestyle='-')
# Formatting the x-axis to show only the month and year
plt.gca().xaxis.set_major_formatter(DateFormatter('%b %Y'))
# Adding labels and title
plt.xlabel('Date')
plt.ylabel('Yield')
plt.title('Actual Open Yield vs Predicted Yield')
plt.legend()
plt.grid(True)
# Rotate date labels for better visibility
plt.xticks(rotation=45)
# Save and display the plot
plot1_path = os.path.join(plot_directory, 'different.png')
plt.tight_layout()
...
plt.title('Daily MSE between Actual Open Yield and Predicted Yield')
plt.legend()
plt.grid(True)
# Rotate date labels for better visibility
plt.xticks(rotation=45)
# Save and display the plot
plot2_path = os.path.join(plot_directory, 'MSE.png')
plt.tight_layout()
...
plt.xlabel('Date')
plt.ylabel('Error Value')
plt.title('Daily MAE between Actual Open Yield and Predicted Yield')
plt.legend()
plt.grid(True)
# Rotate date labels for better visibility
plt.xticks(rotation=45)
# Save and display the plot
plot3_path = os.path.join(plot_directory, 'MAE.png')
plt.tight_layout()
...
plt.xlabel('Date')
plt.ylabel('Error Value')
plt.title('Daily RMSE between Actual Open Yield and Predicted Yield')
plt.legend()
plt.grid(True)
# Rotate date labels for better visibility
plt.xticks(rotation=45)
# Save and display the plot
plot4_path = os.path.join(plot_directory, 'RMSE.png')
plt.tight_layout()
...
plt.xlabel('Date')
plt.ylabel('Error Value (%)')
plt.title('Daily MAPE between Actual Open Yield and Predicted Yield')
plt.legend()
plt.grid(True)
# Rotate date labels for better visibility
plt.xticks(rotation=45)
# Save and display the plot
plot5_path = os.path.join(plot_directory, 'MAPE.png')
plt.tight_layout()
